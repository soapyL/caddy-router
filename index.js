...
server.get('/', async (req, res) => {
    ...
    res.status(200).send(`
        <h1>Containers</h1>
        ${containers.map(container => `<a class='container-buttons' href='${process.env.PORT ? `${process.env.HOST}:${process.env.PORT}` : process.env.host}/containers/${container.Names[0].replace('/', '')}'>${container.Names[0].replace('/', '')}</a>`)}
    `)
})

server.get('/containers/:container', async (req, res) => {
    const containerName = req.params.container
    const reload = await execContainer(containerName, ['caddy', 'reload', `--config ${process.env.CADDY_PATH}/${containerName}.caddy`])
