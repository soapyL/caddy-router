...
server.get('/', async (req, res) => {
    ...
    res.status(200).send(`
        <h1>Containers</h1>
        ${containers.map(container => `<a class='container-buttons' href='${process.env.HOST}:${process.env.PORT}/containers/${container.Names[0].replace('/', '')}'>${container.Names[0].replace('/', '')}</a>`)}
    `)
})
